...
function handleObjectScaling(e) {
  ...
  if (
    scalingObject.isConnector ||
    scalingObject.isConnectorArrow ||
    scalingObject.type === 'textbox'
  ) {
    return;
  }
...
function handleDoubleClick(opt) {
  ...
  if (
    !target ||
    target.type === 'textbox' ||
    (target.type === 'i-text' && !target._isShapeText && !target._isConnectorText)
  ) {
    return;
  }
...
function handleMouseMove(opt) {
  if (isPanning) {
    ...
    canvas.requestRenderAll();
    updateGridPosition(); // Update grid position during panning
    lastPosX = evt.clientX;
    lastPosY = evt.clientY;
    opt.e.preventDefault();
    opt.e.stopPropagation();
...
/**
 * Update grid position based on viewport transform
 */
...
function handleMouseOver(e) {
  ...
  if (
    ...
    target.type === 'textbox' ||
    target.isConnector ||
    target.isConnectorArrow ||
    target.isConnectionHandle
  ) {
    return;
  }
